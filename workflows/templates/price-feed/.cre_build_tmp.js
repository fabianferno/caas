...
var configSchema = exports_external.object({
  ...
  evms: exports_external.array(exports_external.object({
    ...
    consumerContract: exports_external.string(),
    gasLimit: exports_external.string()
  }))
});
var fetchPrice = (sendRequester, config) => {
...
var onCronTrigger = (runtime2, _payload) => {
  ...
  const price = httpClient.sendRequest(runtime2, fetchPrice, consensusMedianAggregation())(runtime2.config).result();
  runtime2.log(`Fetched price: ${price}`);
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 price"), [price]);
